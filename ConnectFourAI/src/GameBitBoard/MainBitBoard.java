...
public class MainBitBoard {
    ...
    /** My attempt at making a bitboard based min/max search.
     Theoretically it is the most efficient way to preform the algorithm */
    static int findMoveBB(BitB P) {
        // check for a draw (first base case)
        if (P.moves >= BitB.HEIGHT * BitB.WIDTH) {
            return 0;
        }

        // check for a win (second base case)
        for (int i = 0; i < BitB.WIDTH; i++) {
            if (P.canPlay(i) && P.isWinningMove(i)) {
                return (BitB.HEIGHT * BitB.WIDTH) - P.moves;
            }
        }
        return 0;
    }
    ...
    static int findMove(int depth, int sense, boolean saveMove, int alpha, int beta) {
        ...
        for (int M : getLegalMovesBetterOrder()) {
            ...
            int valueFromChild = findMove(depth - 1, sense * -1, false, alpha, beta);
    ...
    static int getBestMove() {
        ...
        findMove(AI_DEPTH, sense, true, -INFINITY, INFINITY);
